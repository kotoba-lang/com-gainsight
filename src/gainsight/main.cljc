(ns gainsight.main
  "Kotodama WASM entrypoint for the Gainsight clean-room actor (L4) — Clojure port.
  ...
  facts (`gainsight.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "gainsight")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"gainsight.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "gainsight-compat" :tier tier :entities entities} 200])
...
(defn start! [] :gainsight-compat/ready)
